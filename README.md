...
- `cn_list`: IPs of the CNs (defaults to `server_list` excluding `mn_list`).
...
cd ~/Ditto
git apply ~/forge/patches/ditto-server-data-len-semantics.patch
```

By applying the semantic patch and configuring both FORGE and Ditto with **this artifact's** `settings.py`, we evaluate all systems under the same aligned object caching capacity, ensuring an apples-to-apples comparison.
