...
DMCMemcachedClient::DMCMemcachedClient(const char* memcached_ip) {
  ...
  printd(L_DEBUG, "Connecting to server %s", memcached_ip_);
...
void DMCMemcachedClient::memcached_put_result(void* result,
  ...
  sprintf(key_buf, "client-%d-result-%d", client_id, num_result_);
  memcached_publish(key_buf, result, result_size);
  num_result_++;
}

void DMCMemcachedClient::memcached_sync_ready(uint32_t client_id) {
  assert(client_id >= 1);

  char key_buf[MEMCACHED_MAX_NAME_LEN];
  sprintf(key_buf, "client-%d-ready-%d", client_id, num_sync_);
