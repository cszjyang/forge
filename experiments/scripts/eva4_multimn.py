...
from utils.cluster_manager import ClusterManager, NUM_MN, cluster_ips
...
# Multi-MN experiment.
# In the paper's evaluation, we use 3 CNs and 3 MNs for the multi-MN experiment
# Please manually update experiments/scripts/utils/settings.py::mn_list as needed before running.
method_list = ['forge']
workload_list = ['meta_kvcache_traces_4-100m', 'oG-twitter030-100m']
cache_size_list = ['0.2']
num_c_list = [100, 200, 300]
...
for wl, cache_size in product(workload_list, cache_size_list):
    ...
    for num_c, method in product(num_c_list, method_list):
        cluster_manager.set_run_params(method, wl, cache_size, NUM_MN, num_c, running_option, timeout=timeout, dry_run=args.dry_run)
