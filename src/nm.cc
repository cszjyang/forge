...
int UDPNetworkManager::UDPNMInitClient(const DMCConfig* conf) {
  ...
  dm_mr_info_list_.resize(num_server_);
  ib_send_cq_ = ibv_create_cq(ib_ctx_, 1024, NULL, NULL, 0);
...
UDPNetworkManager::UDPNetworkManager(const DMCConfig* conf) {
  ...
  role_ = conf->role;
  server_id_ = role_ == CLIENT ? conf->client_id : conf->server_id;
  ...
  if (role_ == CLIENT) {
    ...
    client_master_ = ((conf->client_id - conf->st_client_id) % NUM_THREAD_PER_SMM == 0);
  } else {
...
struct ibv_qp* UDPNetworkManager::create_rc_qp() {
  ...
  // ibv_devinfo -v | grep -E "qp|cq|sge"
  qp_init_attr.qp_type = IBV_QPT_RC;
  qp_init_attr.sq_sig_all = 0;
  qp_init_attr.send_cq = ib_send_cq_;
  qp_init_attr.recv_cq = ib_recv_cq_;
  qp_init_attr.cap.max_send_wr = client_master_ ? MAX_SEND_WR_SMM : MAX_SEND_WR;
  qp_init_attr.cap.max_recv_wr = role_ == CLIENT ? 1 : 512;
...
int UDPNetworkManager::server_connect_self_rc_qp() {
  int rc = 0;
  self_connect_send_qp = create_rc_qp();
  assert(self_connect_send_qp != NULL);
  QPInfo send_qp_info;
  rc = get_qp_info(self_connect_send_qp, &send_qp_info);

  struct ibv_qp* self_connect_recv_qp = create_rc_qp();
  assert(self_connect_recv_qp != NULL);
  if (this->rc_qp_list_.size() <= server_id_) {
    this->rc_qp_list_.resize(server_id_ + 1);
  }
  if (this->rc_qp_list_[server_id_] != NULL) {
    ibv_destroy_qp(rc_qp_list_[server_id_]);
  }
  this->rc_qp_list_[server_id_] = self_connect_recv_qp;
  QPInfo recv_qp_info;
  rc = get_qp_info(self_connect_recv_qp, &recv_qp_info);

  ib_print_gid(send_qp_info.gid);
  rc = ib_connect_qp(self_connect_send_qp, &send_qp_info, &recv_qp_info,
                     conn_type_);
  assert(rc == 0);
  ib_print_gid(recv_qp_info.gid);
  rc = ib_connect_qp(self_connect_recv_qp, &recv_qp_info, &send_qp_info,
                     conn_type_);
  assert(rc == 0);
  return 0;
}
...
int UDPNetworkManager::client_connect_one_rc_qp(uint16_t dest_server_id,
  ...
  // record & return the device memory info
  MrInfo* new_dm_mr_info = (MrInfo*)malloc(sizeof(MrInfo));
  memcpy(new_dm_mr_info, &reply.body.conn_info.dm_mr_info, sizeof(MrInfo));
  dm_mr_info_list_[dest_server_id] = new_dm_mr_info;
  memcpy(dm_mr_info, &reply.body.conn_info.dm_mr_info, sizeof(MrInfo));
  return 0;
}
...
int UDPNetworkManager::rdma_write_sid_sync(uint16_t server,
  ...
  if (ret != 0) {
    printd(L_ERROR, "ibv_post_send failed: %s, bad_wr id: %d", strerror(ret), bad_wr->wr_id);
    exit(-1);
  }
...
int UDPNetworkManager::rdma_faa_size_async(uint16_t server,
                                           uint64_t remote_addr,
                                           uint32_t rkey,
                                           uint64_t local_addr,
                                           uint32_t lkey,
...
int UDPNetworkManager::rdma_cas_sid_async(uint16_t server,
                                          uint64_t remote_addr,
                                          uint32_t rkey,
                                          uint64_t local_addr,
                                          uint32_t lkey,
...
int UDPNetworkManager::rdma_poll_send_sid_async(struct ibv_send_wr* sr,
                                                uint16_t server) {
  struct ibv_wc wc;
  memset(&wc, 0, sizeof(struct ibv_wc));
  int ret = rdma_poll_one_send_completion_sync(&wc);
  if (wc.status != IBV_WC_SUCCESS) {
...
int UDPNetworkManager::rdma_poll_send_sid_async() {
  struct ibv_wc wc;
  memset(&wc, 0, sizeof(struct ibv_wc));
  int ret = rdma_poll_one_send_completion_sync(&wc);
  if (wc.status != IBV_WC_SUCCESS) {
...
int UDPNetworkManager::rdma_post_recv_sid_async(struct ibv_recv_wr* rr,
  ...
  if (ret != 0) {
    printd(L_ERROR, "ibv_post_recv return %d %s", ret, strerror(ret));
    exit(-1);
  }
  return 0;
}
