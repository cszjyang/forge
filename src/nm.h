...
class UDPNetworkManager {
 private:
  ...
  std::vector<MrInfo*> dm_mr_info_list_;
 ...
 public:
  ...
  inline uint32_t get_server_dm_rkey(uint8_t server_id) {
    return dm_mr_info_list_[server_id]->rkey;
  }

  inline struct ibv_context* get_ib_ctx() { return ib_ctx_; }
 ...
 public:
  ...
  // for server
  struct ibv_qp* self_connect_send_qp;
  int server_connect_self_rc_qp();

  // for client
  int client_connect_one_rc_qp(uint16_t server_id,
  ...
  int rdma_cas_sid_sync(uint16_t server,
                        ...
                        uint64_t swap_val);
  int rdma_cas_sid_async(uint16_t server,
                         uint64_t remote_addr,
                         uint32_t rkey,
                         uint64_t local_addr,
                         uint32_t lkey,
  ...
  int rdma_faa_size_sync(uint16_t server,
                         ...
                         uint32_t lkey,
                         uint64_t add);
  int rdma_faa_size_async(uint16_t server,
                          uint64_t remote_addr,
                          uint32_t rkey,
                          uint64_t local_addr,
                          uint32_t lkey,
  ...
  int rdma_poll_send_sid_async(struct ibv_send_wr* sr_list, uint16_t server);
  int rdma_poll_send_sid_async();
