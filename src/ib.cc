...
void ib_print_wr(struct ibv_send_wr* wr_list) {
  ...
  for (p = wr_list; p != NULL; p = p->next) {
    ...
    } else if (p->opcode == IBV_WR_ATOMIC_FETCH_AND_ADD) {
      printd(L_INFO,
             "wr_id: %ld, opcode: %d, raddr: 0x%lx, rkey: 0x%x, add: 0x%lx",
             p->wr_id, p->opcode, p->wr.atomic.remote_addr, p->wr.atomic.rkey,
             p->wr.atomic.compare_add);
    } else {
      printd(L_INFO, "wr_id: %ld, opcode: %d", p->wr_id, p->opcode);
    }
  }
}